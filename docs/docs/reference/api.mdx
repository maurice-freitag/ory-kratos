...
<a id="opIdinitializeSelfServiceLoginForNativeApps"></a>
...
curl -X POST /self-service/settings?flow=string \
  -H 'Content-Type: application/json' \  -H 'Accept: application/json' \  -H 'X-Session-Token: string' \  -H 'X-Session-Token: API_KEY'
```
...
curl -X GET /self-service/settings/api \
  -H 'Accept: application/json' \  -H 'X-Session-Token: string' \  -H 'X-Session-Token: API_KEY'
```
...
```shell
curl -X GET /sessions/whoami \
  -H 'Accept: application/json' \  -H 'X-Session-Token: string' \  -H 'Cookie: API_KEY'
```
